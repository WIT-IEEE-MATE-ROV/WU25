...
uint32_t BNO08x::rx_buflen;
uint32_t BNO08x::tx_buflen;
...
const char *spi_state_str(SpiState_t state)
{
    ...
    switch (state) {
        case SPI_INIT: return STR(SPI_INIT);
        case SPI_DUMMY: return STR(SPI_DUMMY);
        case SPI_DFU: return STR(SPI_DFU);
        case SPI_IDLE: return STR(SPI_IDLE);
        case SPI_RD_HDR: return STR(SPI_RD_HDR);
        case SPI_RD_BODY: return STR(SPI_RD_BODY);
        case SPI_WRITE: return STR(SPI_WRITE);
        default: return "Undefined SPI State";
    }
...
void BNO08x::update_sequence_number(packet_header *hdr) {
    sequence_numbers[hdr->channel] = hdr->seq_num;
}

Packet BNO08x::read_packet_type(bno_channel channel, float timeout_ms)
{
    auto start_time = std::chrono::high_resolution_clock::now();
...
uint32_t BNO08x::read_header(packet_header *hdr_out, bool wait_for_int)
{
...
std::string shtpevent_to_str(sh2_ShtpEvent_t event)
{
    ...
    switch (event) {
        case SH2_SHTP_TX_DISCARD: return STR(SH2_SHTP_TX_DISCARD);
        case SH2_SHTP_SHORT_FRAGMENT: return STR(SH2_SHTP_SHORT_FRAGMENT);
        case SH2_SHTP_TOO_LARGE_PAYLOADS: return STR(SH2_SHTP_TOO_LARGE_PAYLOADS);
        case SH2_SHTP_BAD_RX_CHAN: return STR(SH2_SHTP_BAD_RX_CHAN);
        case SH2_SHTP_BAD_TX_CHAN: return STR(SH2_SHTP_BAD_TX_CHAN);
        case SH2_SHTP_BAD_FRAGMENT: return STR(SH2_SHTP_BAD_FRAGMENT);
        case SH2_SHTP_BAD_SN: return STR(SH2_SHTP_BAD_SN);
        case SH2_SHTP_INTERRUPTED_PAYLOAD: return STR(SH2_SHTP_INTERRUPTED_PAYLOAD);
        default: return "Unknown sh2_ShtpEvent_t";
    }
...
int32_t BNO08x::read_sh2_cb(sh2_Hal_t *self, uint8_t *pBuffer, unsigned len, uint32_t *t_us)
{
    // printf("Read cb\n");
...
int32_t BNO08x::write_sh2_cb(sh2_Hal_t *self, uint8_t *pBuffer, unsigned len)
{
    // printf("Write cb\n");
    int32_t ret = SH2_OK;

    if ((self == 0) || (len > SPI_BUFLEN) || ((len > 0) && (pBuffer == 0))) {
